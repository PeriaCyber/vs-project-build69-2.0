...
void CEntity::Add()
{
	...
	if(!*(uintptr_t*)((uintptr_t)m_pEntity + 0xB8)) 
	{
		...
		// CWorld::Add
		((void(*)(uintptr_t))(g_libGTASA + 0x4233C8 + 1))((uintptr_t)m_pEntity);
...
void CEntity::Remove()
{
	...
	{
		// CWorld::Remove
		((void(*)(uintptr_t))(g_libGTASA + 0x4232BC + 1))((uintptr_t)m_pEntity);
	}
}
...
{
	if(m_pEntity && m_pEntity->vtable != g_libGTASA + 0x667D24)
	{
		if(*(uintptr_t *)((uintptr_t)m_pEntity + 0x18))
		{
			...
			if(mat)
			{
				...
				((void (*) (MATRIX4X4*, uintptr_t))(g_libGTASA + 0x44EDEE))(mat, pRwMatrix);

				// CEntity::UpdateRwFrame
				((void (*) (ENTITY_TYPE*))(g_libGTASA + 0x3EBFE8))(m_pEntity);
			}
		}
	}
}
...
void CEntity::Render()
{
	...
	if(iModel >= 400 && iModel <= 611 && *(uintptr_t *)((uintptr_t)m_pEntity + 0x18))
	{
		...
		((void (__fastcall *)(uintptr_t))(g_libGTASA + 0x5D4B41))(*(uintptr_t *)((uintptr_t)m_pEntity + 0x18));
	}
	...
	(( void (*)(ENTITY_TYPE*))(g_libGTASA + 0x4102BC))(m_pEntity);
}
...
void CEntity::SetMoveSpeedVector(VECTOR Vector)
{
	...
	if(!IsValidVector(Vector)) {
		Log("[1] Invalid Speed Vector >> %f %f %f", 
...
void CEntity::SetTurnSpeedVector(VECTOR Vector)
{
	...
	if(!IsValidVector(Vector)) {
		Log("[2] Invalid Speed Vector >> %f %f %f", 
			Vector.X, Vector.Y, Vector.Z);
...
bool CEntity::IsAdded()
{
	if(m_pEntity)
	{
		if(m_pEntity->vtable == g_libGTASA + 0x667D24) {
			return false;
		}
...
bool CEntity::SetModelIndex(unsigned int uiModel)
{
	...
	if(!pGame->IsModelLoaded(uiModel) && !IsValidModel(uiModel))
	{
		...
		while(!pGame->IsModelLoaded(uiModel))
		{
			...
			{
				if(pChat) {
					pChat->AddDebugMessage("Warning: Model %u wouldn't load in time!", uiModel);
				}
...
float CEntity::GetDistanceFromCamera()
{
	...
	float tmpX = (matEnt.pos.X - pCam->pos1.X); // *(float*)(g_libGTASA + 0x9528D4)
	float tmpY = (matEnt.pos.Y - pCam->pos1.Y); // *(float*)(g_libGTASA + 0x9528D8)
	float tmpZ = (matEnt.pos.Z - pCam->pos1.Z); // *(float*)(g_libGTASA + 0x9528DC)
