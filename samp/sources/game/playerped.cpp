...
{
	(( void (*)(uintptr_t))(g_libGTASA + 0x4C3745))((uintptr_t)m_pPed);
}
...
void CPlayerPed::PutDirectlyInVehicle(int iVehicleID, int iSeat)
{
	...
	if (pVehicle->entity.vtable == g_libGTASA + 0x667D24) return;
...
void CPlayerPed::SetModelIndex(unsigned int uiModel)
{
	...
	if(m_pPed)
	{
		...
		(( void (*)(uintptr_t, uintptr_t))(g_libGTASA + 0x39CE69))(((uintptr_t)m_pPed + 0x298), (uintptr_t)m_pPed);
	}
}
...
{
	if(!m_pPed) return;
	if(m_pEntity->vtable == g_libGTASA + 0x667D24) return;

	(( void (*)(PED_TYPE*, VECTOR*, int, int))(g_libGTASA + 0x4A4A9D))(m_pPed, vecOut, iBoneID, 0);
}
...
void CPlayerPed::ClumpUpdateAnimations(float step, int flag)
{
	if(m_pPed)
	{
		...
		if(pRwObj)
		{
			((void (*)(uintptr_t, float, int))(g_libGTASA + 0x38BF01))(pRwObj, step, flag);
		}
	}
}
...
void CPlayerPed::GetBonePosition(VECTOR &out, unsigned int boneid, bool update)
{
	...
	if(m_pPed)
	{
		...
		((void(*)(uintptr_t, VECTOR &, unsigned int, bool))(g_libGTASA + 0x4A4A9C + 1))((uintptr_t)m_pPed, out, boneid, update);
	}
}
...
{
	if(!m_pPed) return false;
	if(m_pPed->entity.vtable == g_libGTASA + 0x667D24) return false;
	...
	uintptr_t pAnimHierarchy = (( uintptr_t (*)(uintptr_t))(g_libGTASA + 0x5D1020 + 1))(pdwRenderWare);

	// RpHAnimIDGetIndex(RpHAnimHierarchy *,int)	.text	001C2C90	00000024			R	.	.	.	.	.	.
	int iAnimIndex = (( uintptr_t (*)(uintptr_t, uintptr_t))(g_libGTASA + 0x1C2C90 + 1))(pAnimHierarchy, iBoneID) << 6;
...
void CPlayerPed::UpdateAttachedObject(bool create, uint32_t index, uint32_t model, uint32_t bone, VECTOR vecOffset, VECTOR vecRotation, VECTOR vecScale, uint32_t materialcolor1, uint32_t materialcolor2)
{
    ...
    if(create)
    {
        if(m_pPed->entity.vtable == g_libGTASA + 0x667D24) return;
