...
{
	((int (*)())(g_libGTASA + 0x2FE621))();
  	((int (*)())(g_libGTASA + 0x2FE5BB))();
  	ARMHook::makeRET(g_libGTASA + 0x41552C);
}
...
void CGame::ToggleThePassingOfTime(int iState)
{
	if(iState)
	{
		ARMHook::writeMemory(g_libGTASA + 0x3E3378, (uintptr_t)"\xD0\xB5", 2);
	}
	else
	{
		ARMHook::makeRET(g_libGTASA + 0x3E3378);
	}
}
...
{
	*(uint8_t *)(g_libGTASA + 0x953143) = (uint8_t)iMinute;
  	*(uint8_t *)(g_libGTASA + 0x953142) = (uint8_t)iHour;
...
{
	*(unsigned char*)(g_libGTASA + 0xA7D138) = byteWeatherID;

	if(!m_bClockEnabled)
	{
		*(uint16_t*)(g_libGTASA + 0xA7D134) = byteWeatherID;
		*(uint16_t*)((g_libGTASA + 0xA7D134) + 2) = byteWeatherID;
	}
}
...
void CGame::EnableClock(bool bEnable)
{
	...
	ARMHook::unprotect(g_libGTASA + 0x2BD5A8);

	if(bEnable)
	{
		ToggleThePassingOfTime(1);
		m_bClockEnabled = true;
		memcpy((void*)(g_libGTASA + 0x2BD5A8), byteClockData, 10);
	}
	else
	{
		ToggleThePassingOfTime(0);
		m_bClockEnabled = false;
		memset((void*)(g_libGTASA + 0x2BD5A8), 0, 10);
	}
}
...
{
	*(uint8_t *)(g_libGTASA + 0x991FD8) = (uint8_t)!iToggle; // CHud::bScriptDontDisplayRadar
}

void CGame::DisplayHUD(bool bDisp)
{
	*(uint8_t *)(g_libGTASA + 0x819D88) = (uint8_t)bDisp; // CTheScripts::bDisplayHud
  	*(uint8_t *)(g_libGTASA + 0x991FD8) = (uint8_t)!bDisp; // CHud::bScriptDontDisplayRadar
}
...
{
	ARMHook::writeMemory(g_libGTASA + 0x2BDF6C, (uintptr_t)&byteLevel, 1);
}
...
void CGame::DisplayGameText(char* szStr, int iTime, int iType)
{
	...
	(( void (*)(uint16_t*, int, int))(g_libGTASA + 0x54C5BD))(szGameTextMessage, iTime, iType);
}

// 0.3.7
void CGame::SetGravity(float fGravity)
{
	ARMHook::unprotect(g_libGTASA + 0x3FE7C0);
	*(float*)(g_libGTASA + 0x3FE7C0) = fGravity;
}
...
{
	uintptr_t addr = *(uintptr_t*)(g_libGTASA + 0x7A1E18);
...
void CGame::DrawGangZone(float fPos[], uint32_t dwColor)
{
    ...
    (( void (*)(float*, uint32_t*, uint8_t))(g_libGTASA + 0x443C10 + 1))(fPos, &dwColor, bGZ);
}
	...
	if (iModel >= 0 && iModel < 20000)
	{
		if (bFromStreaming)
		{
			if(ScriptCommand(&is_model_available, iModel)) 
			{
				...
				((void(*)(int))(g_libGTASA + 0x2D00B9))(iModel);
			}
		}
		else
		{
