...
void KeyBoard::Render()
{
	...
	float m_fButWidth = ImGui::CalcTextSize("QWE").x;
	float m_fButHeight = ImGui::CalcTextSize("QWE").x;

	ImGui::SetNextWindowPos(ImVec2(io.DisplaySize.x / 1.1735 - m_fButWidth, io.DisplaySize.y / 1.80 - m_fButHeight * 3.0));
	...
	//input string
	if(!m_bHidenInput)
		ImGui::GetOverlayDrawList()->AddText(pUI->getFont(), m_fFontSize, 
	...
	//dividing line
	ImGui::GetOverlayDrawList()->AddLine(ImVec2(m_Pos.x, m_Pos.y + m_fKeySizeY), ImVec2(m_Size.x, m_Pos.y + m_fKeySizeY), CLIENT_COLOR);
	...
	for(int i=0; i<LAYOUT_ROWS_C; i++)
	{
		...
		//for( auto key : m_Rows[m_iLayout][i])
		{
			...
			if(key.id == m_iPushedKey && key.type != KEY_SPACE && !m_bHidenInput)
				ImGui::GetOverlayDrawList()->AddRectFilled(key.pos, ImVec2(key.pos.x + key.width, key.pos.y + fKeySizeY), CLIENT_COLOR);

			switch(key.type)
			{
				case KEY_DEFAULT:
					ImGui::GetOverlayDrawList()->AddText(pUI->getFont(), m_fFontSize, key.symPos, 0xFFFFFFFF, key.name[m_iCase]);
				break;

				case KEY_SHIFT:
					ImGui::GetOverlayDrawList()->AddTriangleFilled(ImVec2(key.pos.x + key.width * 0.37, key.pos.y + fKeySizeY * 0.50), ImVec2(key.pos.x + key.width * 0.50, key.pos.y + fKeySizeY * 0.25), ImVec2(key.pos.x + key.width * 0.63, key.pos.y + fKeySizeY * 0.50), m_iCase == LOWER_CASE ? 0xFF8A8886 : CLIENT_COLOR);
					ImGui::GetOverlayDrawList()->AddRectFilled(ImVec2(key.pos.x + key.width * 0.44, key.pos.y + fKeySizeY * 0.5 - 1), ImVec2(key.pos.x + key.width * 0.56, key.pos.y + fKeySizeY * 0.68), m_iCase == LOWER_CASE ? 0xFF8A8886 : CLIENT_COLOR);
				break;
				...
				case KEY_SWITCH:
					if(m_iLayout == 0)
						ImGui::GetOverlayDrawList()->AddText(
							pUI->getFont(), m_fFontSize, key.symPos, 0xFFFFFFFF, "123");
					else if(m_iLayout == 1)
						ImGui::GetOverlayDrawList()->AddText(
							pUI->getFont(), m_fFontSize, key.symPos, 0xFFFFFFFF, "abc");
				break;

				case KEY_SPACE:
					ImGui::GetOverlayDrawList()->AddRectFilled(ImVec2(key.pos.x + key.width * 0.07, key.pos.y + fKeySizeY * 0.3), ImVec2(key.pos.x + key.width * 0.93, key.pos.y + fKeySizeY * 0.7), key.id == m_iPushedKey ? CLIENT_COLOR : 0xFF8A8886);
				break;
...
void KeyBoard::addTextToBuffer(std::string msg)
{
	for(int i = 0; i < msg.size(); i++)
		AddCharToInput((char)msg[i]);
}

void KeyBoard::Open(keyboard_callback* handler, bool hiden)
{
	if(handler == nullptr) 
		return;
...
bool KeyBoard::onTouchEvent(int type, int x, int y)
{
	if(!m_bEnable) return true;

	ImGuiIO& io = ImGui::GetIO();
	
	if(x >= (io.DisplaySize.x / 1.1735) - (ImGui::GetFontSize() * 4) && y >= io.DisplaySize.y / 1.80 - (ImGui::GetFontSize() * 2.5) * 3 && y <=  io.DisplaySize.y / 1.80) 
		return true;
...
void KeyBoard::HandleInput(kbKey &key)
{
	switch(key.type)
	{
		case KEY_DEFAULT:
...
void KeyBoard::AddCharToInput(char sym)
{
	...
	check:
		ImVec2 textSize = pUI->getFont()->CalcTextSizeA(m_fFontSize, FLT_MAX, 0.0f,  m_utf8Input, nullptr, nullptr);
...
void KeyBoard::DeleteCharFromInput()
{
	...
	textSize = pUI->getFont()->CalcTextSizeA(m_fFontSize, FLT_MAX, 0.0f,  m_utf8Input, nullptr, nullptr);
...
void KeyBoard::InsertText(const std::string &string)
{
	m_sInput.clear();
	m_utf8Input[0] = '\0';
	for(char ch : string)
...
void KeyBoard::Send()
{
	...
	{
		// c++_static
		if(m_sInput.size() && !pDialog->GetState())
		{
			m_iCurrentPosItem = 0;
			m_iTotalLastItem++;
...
kbKey* KeyBoard::GetKeyFromPos(int x, int y)
{
