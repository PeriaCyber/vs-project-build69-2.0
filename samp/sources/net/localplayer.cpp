...
{
	if(pChat) {
		// pChat->AddDebugMessage("CLocalPlayer::Process");
	}
	...
	{
		// if(pChat) pChat->AddDebugMessage("CLocalPlayer::Process #1");

		// handle dead
		if(!m_bIsWasted && m_pPlayerPed->GetActionTrigger() == ACTION_DEATH || m_pPlayerPed->IsDead())
		{
			// if(pChat) pChat->AddDebugMessage("CLocalPlayer::Process #2");
		...
		// if(pChat) pChat->AddDebugMessage("CLocalPlayer::Process #3");
		...
		if(m_bIsSpectating)
		{
			// if(pChat) pChat->AddDebugMessage("CLocalPlayer::Process #4");
			ProcessSpectating();
		}
		// DRIVER
		else if(m_pPlayerPed->IsInVehicle() && !m_pPlayerPed->IsAPassenger())
		{
			// if(pChat) pChat->AddDebugMessage("CLocalPlayer::Process #5");
			...
			{
				// if(pChat) pChat->AddDebugMessage("CLocalPlayer::Process #6");
		...
		{
			// if(pChat) pChat->AddDebugMessage("CLocalPlayer::Process #7");
			...
			{
				// if(pChat) pChat->AddDebugMessage("CLocalPlayer::Process #8");
		...
		{
			// pChat->AddDebugMessage("CLocalPlayer::Process #9");

			if((dwThisTick - m_dwLastSendTick) > (unsigned int)GetOptimumInCarSendRate())
			{
				// if(pChat) pChat->AddDebugMessage("CLocalPlayer::Process #10");
...
bool CLocalPlayer::HandlePassengerEntry()
{
    ...
    int isHoldDown = 0;//(( int (*)(int, int, int))(g_libGTASA + 0x2B2068 + 1))(0, 1, 1);
...
bool CLocalPlayer::Spawn()
{
	...
	// CCamera::Fade
	ARMHook::writeMemory(g_libGTASA + 0x3DB5E0, (uintptr_t)"\x70\x47", 2); // bx lr
