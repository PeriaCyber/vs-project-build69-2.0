...
CNetGame::CNetGame(const char* szHostOrIp, int iPort, const char* szPlayerName, const char* szPass)
{
	...
	if(pChat)
	{
		pChat->AddDebugMessage("{FFFFFF}");
		pChat->AddDebugMessage("{FFFFFF}SA-MP {B9C9BF}" SAMP_VERSION " {FFFFFF}Started");
	}
}
...
{
	if(pChat) 
		pChat->AddDebugMessage("Connecting to %s:%d...", m_szHostOrIp, m_iPort);
...
{
	/*if(pChat) {
		pChat->AddDebugMessage("CNetGame::ProcessPools");
	}*/

	if(m_pPlayerPool) 
	{
		/*if(pChat) {
			pChat->AddDebugMessage("CPlayerPool::Process");
		}*/
	...
	{
		/*if(pChat) {
			pChat->AddDebugMessage("CVehiclePool::Process");
		}*/
	...
	{
		/*if(pChat) {
			pChat->AddDebugMessage("CPickupPool::Process");
		}*/
...
{
	if(pChat) {
		pChat->AddInfoMessage("{81AF66}The server is restarting..");
	}
...
{
	if(pChat) {
		pChat->AddDebugMessage("The server didn't respond. Retrying..");
	}
...
{
	if(pChat) {
		pChat->AddDebugMessage("The server is full. Retrying...");
	}
...
{
	if(pChat) {
		pChat->AddDebugMessage("Failed to initialize encryption.");
	}
}
...
{
	if(pChat) {
		pChat->AddDebugMessage("You are banned from this server.");
	}
}
...
{
	if(pChat) {
		pChat->AddDebugMessage("Wrong server password.");
	}
...
{
	if(pChat)
		pChat->AddDebugMessage("Server closed the connection.");
...
void CNetGame::Packet_ConnectionLost(Packet* pkt)
{
	...
	if(pChat)
		pChat->AddDebugMessage("Lost connection to the server. Reconnecting..");
...
{
	if(pChat) 
		pChat->AddDebugMessage("Connected. Joining the game...");
