...
void InitGame(RPCParameters *rpcParams)
{
	...
	if(pChat) 
		pChat->AddDebugMessage("Connected to {B9C9BF}%.64s", pNetGame->m_szHostName);
}
...
void ClientMessage(RPCParameters *rpcParams)
{
	...
	//Log(szMsg);
	if(pChat) pChat->AddClientMessage(dwColor, szMsg);
...
void Chat(RPCParameters *rpcParams)
{
	...
	if (playerId == pPlayerPool->GetLocalPlayerID())
	{
		...
		if (pLocalPlayer) 
		{
			if(pChat) pChat->AddChatMessage(pPlayerPool->GetLocalPlayerName(),
...
void DialogBox(RPCParameters *rpcParams)
{
	...
	if(pDialog)
	{
		...
		pDialog->m_wDialogID = wDialogID;
		pDialog->m_byteDialogStyle = byteDialogStyle;

		// title
		bsData.Read(len);
		bsData.Read(szBuff, len);
		szBuff[len] = '\0';
		cp1251_to_utf8(pDialog->m_utf8Title, szBuff);

		// button1
		bsData.Read(len);
		bsData.Read(szBuff, len);
		szBuff[len] = '\0';
		cp1251_to_utf8(pDialog->m_utf8Button1, szBuff);

		// button2
		bsData.Read(len);
		bsData.Read(szBuff, len);
		szBuff[len] = '\0';
		cp1251_to_utf8(pDialog->m_utf8Button2, szBuff);

		// info
		stringCompressor->DecodeString(szBuff, 4096, &bsData);
		pDialog->SetInfo(szBuff, strlen(szBuff));

		if(wDialogID < 0 || wDialogID == 65535)
			return;

		if( pDialog->m_utf8Title[0] == '\0' && pDialog->m_utf8Button1[0] == '\0' && pDialog->m_utf8Button2[0] == '\0' && pDialog->m_utf8Title[0] == '\0' ) // sampapk
			return;
...
void ConnectionRejected(RPCParameters *rpcParams)
{
	...
	if(byteRejectReason == REJECT_REASON_BAD_VERSION) {
		if(pChat) pChat->AddInfoMessage("{81AF66}CONNECTION REJECTED: Incorrect Version.");
	}
	else if(byteRejectReason == REJECT_REASON_BAD_NICKNAME)
	{
		if(pChat) pChat->AddInfoMessage("{81AF66}CONNECTION REJECTED: Unacceptable NickName");
		if(pChat) pChat->AddInfoMessage("{81AF66}Please choose another nick between and 3-20 characters");
		if(pChat) pChat->AddInfoMessage("{81AF66}Please use only a-z, A-Z, 0-9");
		if(pChat) pChat->AddInfoMessage("{81AF66}Use /quit to exit or press ESC and select Quit Game");
	}
	else if(byteRejectReason == REJECT_REASON_BAD_MOD)
	{
		if(pChat) pChat->AddInfoMessage("{81AF66}CONNECTION REJECTED: Bad mod version.");
	}
	else if(byteRejectReason == REJECT_REASON_BAD_PLAYERID)
	{
		if(pChat) pChat->AddInfoMessage("{81AF66}CONNECTION REJECTED: Unable to allocate a player slot.");
	}
