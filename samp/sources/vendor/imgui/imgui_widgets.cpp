...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    const ImRect inner_rect = window->InnerRect;
    const float size_avail = inner_rect.Max[axis] - inner_rect.Min[axis];
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        float mouse_pos_v = (axis == ImGuiAxis_X) ? g.IO.MousePos.x : g.IO.MousePos.y;
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse)
        {
            ...
            {
                const float mouse_abs_pos = (axis == ImGuiAxis_X) ? g.IO.MousePos.x : g.IO.MousePos.y;
