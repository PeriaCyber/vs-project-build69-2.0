	...
	for(uint16_t playerId = 0; playerId < MAX_PLAYERS; playerId++)
	{
		...
		if(pPlayerPed->GetDistanceFromCamera() <= pNetGame->m_fNameTagDrawDistance)
		{
			...
			if(pNetGame->m_bNameTagLOS) {
				dwHitEntity = ScriptCommand(&get_line_of_sight, VecPos.X, VecPos.Y, VecPos.Z, pCam->pos1.X, pCam->pos1.Y, pCam->pos1.Z, 1, 0, 0, 1, 0);
			}
...
void PlayerTags::Draw(VECTOR* vec, char* szName, uint32_t dwColor, float fDist, float fHealth, float fArmour, bool bAfk)
{
	VECTOR TagPos;
	...
	pUI->drawText(pos, __builtin_bswap32(dwColor | (0x000000FF)), true, szName);
	...
	float fWidth = pUI->scaleX( 100.0f );
	float fHeight = pUI->scaleY( 13.75f );
	float fOutline = 2.0f;

	// top left
	HealthBarBDR1.x = Out.X - ((fWidth/2) + fOutline);
	HealthBarBDR1.y = Out.Y + (pUI->getFontSize() * 1.2f);

	// bottom right
	HealthBarBDR2.x = Out.X + ((fWidth/2) + fOutline);
	HealthBarBDR2.y = Out.Y + (pUI->getFontSize() * 1.2f) + fHeight;
	...
	{
		ImVec2 a = ImVec2(HealthBarBDR1.x - (pUI->getFontSize()*1.4f), HealthBarBDR1.y);
		ImVec2 b = ImVec2(a.x + (pUI->getFontSize()*1.3f), a.y + (pUI->getFontSize()*1.3f));
		...
		uv.x2 = 1.0;
		uv.y2 = 1.0;
		DrawRaster(&rect, 0xFFFFFFFF, m_pAfk_icon->raster, &uv);
	}
}